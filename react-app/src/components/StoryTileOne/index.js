...
const StoryTileOne = ({ story, index }) => {
  ...
  const [date, setDate] = useState('Apr 20, 2420')
  const readTime = Math.floor(Math.random() * (20) + 4);
    ...
    >
      ...
      <div className="style1-content">
        ...
        <div className="style1-story-title-container">
          <div className=" style1-story-title memo-text" onClick={() => history.push(`/story/${story.id}`)}>{story?.title}</div>
        </div>
